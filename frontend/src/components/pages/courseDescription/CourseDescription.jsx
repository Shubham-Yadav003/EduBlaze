...
function CourseDescription({ user }) {
    ...
    const checkoutHandler = async () => {
        ...
        const {  // creates a order with razorpay
          data: { order },
        } = await axios.post(
        ...
        const razorpay = new window.Razorpay(options);  // Create a new Razorpay instance with the options
    ...
    return (
        <>
        {
            ...
            {course && <div className='course-description'>
                ...
                {
                    user && (user.subscription.includes(course._id) || user.mainrole === "superadmin") ? <button onClick={() => navigate(`/course/study/${course._id}`)} className='common-btn1'> Study </button> :
