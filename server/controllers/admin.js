...
export const addLectures = TryCatch(async (req, res) => {
  ...
  // Only course creator or superadmin can add lectures
  if (course.createdBy.trim().toLowerCase() !== req.user.name.trim().toLowerCase() && req.user.mainrole !== "superadmin") {
...
export const deleteLecture = TryCatch(async (req, res) => {
  ...
  // Only course creator or superadmin can delete lectures
  if (course.createdBy.trim().toLowerCase() !== req.user.name.trim().toLowerCase() && req.user.mainrole !== "superadmin") {
    return res.status(403).json({ message: "You are not authorized to delete lectures from this course" });
  }
...
export const deleteCourse = TryCatch(async (req, res) => {
  ...
  // Only course creator or superadmin can delete course
  if (course.createdBy.trim().toLowerCase() !== req.user.name.trim().toLowerCase() && req.user.mainrole !== "superadmin") {
